...
#To the enemy
execute as @a[tag=tasobot.combat,tag=!tasobot.pvp] at @s if entity @n[type=#monsters,distance=..24] run puppet @s[predicate=!vlvtrm:flying] actions run minecraft:move_to entity @n[type=#monsters]
execute as @a[tag=tasobot.pvp] at @s run puppet @s[predicate=!vlvtrm:flying] actions run minecraft:move_to entity @p[tag=!tasobot,tag=!tasobot.ignore]
